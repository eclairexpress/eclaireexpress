...
function compileData (main, housing, jobList, submissions, memories, items, npcs) {
	...
	memoriesRows.forEach(function(row) { 
		var rowKeys = Object.keys(row).filter(function(name) { return /^gsx\$/.test(name) && name !== "gsx$user" }),
			user = row['gsx$user'].$t,
			key;
...
function buildUserPages(mainRows) {
	mainRows.forEach(function(row) {
		var rowUsername = row['gsx$username'].$t,
			rowImg = row['gsx$img'].$t !== "" ? row['gsx$img'].$t : "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/d6155b58-5126-439a-ade3-895083fe1973/dddubwa-17fbff18-e86e-4765-a4e6-e459b24cdac6.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..vtGW91heGJBVAs3ZccbOIVXhSuQ1jXqNDzck09P7IhM",
...
function getHousing(housingObj, characterName) {
	var needsDB = housingObj.indexOf("/") > -1 ? true : false,
