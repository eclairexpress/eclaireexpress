...
}, characterDB = {}, housingDB = {}, jobDB = {}, itemDB = {}, bdayDB = [], 
// for stats
characterCount = 0, hybridCount = 0, npcCount = 0, bdayCount = {
    0: 0,
    1: 0,
    2: 0,
    3: 0
...
function compileData(main, housing, jobList, submissions, memories, items, npcs) {
    ...
    createBdayDB();
    var mainFeed = main[0].feed, housingFeed = housing[0].feed, jobFeed = jobList[0].feed, submissionsFeed = submissions[0].feed, memoriesFeed = memories[0].feed, itemsFeed = items[0].feed, npcsFeed = npcs[0].feed, mainRows = mainFeed.entry || [], housingRows = housingFeed.entry || [], jobRows = jobFeed.entry || [], submissionsRows = submissionsFeed.entry || [], memoriesRows = memoriesFeed.entry || [], itemsRows = itemsFeed.entry || [], npcsRows = npcsFeed.entry || [];
    createJobDB(jobRows);
    ...
    memoriesRows.forEach(function (row) {
        var rowKeys = Object.keys(row).filter(function (name) { return /^gsx\$/.test(name) && name !== "gsx$user"; }), user = row['gsx$user'].$t, key;
    ...
    submissionsRows.forEach(function (row) {
        var users = (row['gsx$usernames'].$t).split(','), date = row['gsx$date'].$t, title = row['gsx$title'].$t, gold = parseInt(row['gsx$gold'].$t), link = row['gsx$submission'].$t, linkApproved = false;
        if (title !== "") {
            linkApproved = true;
        }
        else {
    ...
    var spring = getPercentage(bdayCount[0]), summer = getPercentage(bdayCount[1]), fall = getPercentage(bdayCount[2]);
    ...
    var tt = getPercentage(housingCount["tt"]), ll = getPercentage(housingCount["ll"]), ff = getPercentage(housingCount["ff"]);
...
function addNpcsToCharacters(npcsRows) {
    npcsRows.forEach(function (row) {
        var characterName = row['gsx$name'].$t;
        var characterApp = row['gsx$app'].$t;
        for (var i = 1; i <= 3; i++) {
            var link = row['gsx$link' + i].$t;
            if (link) {
                var characterLink = link.split(",");
                var linkName = characterLink[0].toLowerCase();
                var linkRelationship = characterLink[1];
                var characterObj = characterDB[linkName];
                if (characterObj) {
...
function buildShopPages() {
    ...
    Object.keys(jobDB).forEach(function (rowKey) {
        ...
        $('a#userCell', div).on('click', function (ev) {
            var shopkey = ev.currentTarget.dataset['id'], pageContent = appendShopInfo(shopkey);
...
function createHouseDB(houseRows) {
    ...
    houseRows.forEach(function (row) {
        var rowAddress = row['gsx$address'].$t, rowAddressArray = rowAddress.split('/');
...
function createShopsPage() {
    ...
    Object.keys(itemDB).forEach(function (key) {
        ...
        if (buildingName === "puff-puff pet mart") {
            additionalInfo = "Common farm animals may be purchased at the ranch.";
        }
        ...
        Object.keys(items).forEach(function (itemName) {
            var item = items[itemName], itemDiv = $('<div></div>');
    ...
    for (var i = 0; i < 4; i++) {
        ...
        for (var j = 0; j < 7; j++) {
            ...
            td.on('click', function (ev) {
                var season = $(this).attr('data-season'), day = $(this).attr('data-day'), pageContent = getBdayDiv(season, day);
...
function getBdayDiv(season, day) {
    var bdayArray = bdayDB[season][day], birthdate = parseBirthday([parseInt(season) + 1 + "", parseInt(day) + 1]) + " birthdays";
    if (bdayArray.length === 0) {
        return "";
    }
    else {
        var appLink = characterDB[bdayArray[0]].app, bdayString = "<a href=\"" + appLink + "\" target=\"_blank\">" + bdayArray[0] + "</a>";
...
function getCharacterArray(row, enrollNum) {
    var characters = {}, characterNum, characterString, characterArray, characterName, characterBirthday;
    for (var i = 1; i <= 10; i++) {
...
function getJob(jobArray, name) {
    ...
    var job = jobDB[jobArray[0]]["job" + jobArray[1]];
    job.characters.push(name);
    return job["name"];
}
function getHousing(housingObj, characterName) {
    var needsDB = housingObj.indexOf("/") > -1 ? true : false, housingArray = housingObj.split('/'), housing, residentsString;
    // get the count first
...
function appendShopInfo(key) {
    var jobData = jobDB[key], buildingName = jobData.building, buildingDesc = jobData.desc, buildingImage = jobData.img, buildingImageArtist = jobData.artist, buildingHours = jobData.hours, buildingUniform = jobData.uniform, buildingArea = jobData.area, characterData = "", characterObj = {}, job, needBottom = ["lbl", "tth", "bmbh", "tmth", "cc"], div = document.createElement('div');
...
function appendMemoryInfo(memoriesList) {
    var memoryKeys = Object.keys(memoriesList), collectedMemoriesList = "", key, memory;
    for (var i = 0; i < memoryKeys.length; i++) {
        key = memoryKeys[i];
...
function appendNpcsInfo(npcsObj) {
    var templateString = "";
    Object.keys(npcsObj).forEach(function (npcRelationship) {
        var npcs = npcsObj[npcRelationship];
        var npcsInfo = [];
...
function parseLocation(location, characterName, residentTitle) {
    if (characterName === void 0) { characterName = null; }
    if (residentTitle === void 0) { residentTitle = null; }
    var template, housemates, address;
...
function getAddress(addressArray, numOnly) {
    if (numOnly === void 0) { numOnly = false; }
    var location, number = addressArray[1] === "cc" ? "Captain's Quarters" : addressArray[1];
    if (numOnly) {
        return number;
    }
...
function getHousemates(housematesString, characterName, residentTitle, isCommunal) {
    if (characterName === void 0) { characterName = null; }
    if (residentTitle === void 0) { residentTitle = null; }
    if (isCommunal === void 0) { isCommunal = false; }
    var housemateArray = housematesString.split(','), newHousemateList = "", housemateUrl, housemateFullLink, housemateCount = 0;
    if (housemateArray[0] === "") {
        return "";
    }
...
function parseUpgrades(locationObj) {
    var hasPaint = locationObj.hasPaint === "0" ? "" : "<li>Paint Job</li>", hasDesign = locationObj.hasDesign === "0" ? "" : "<li>House Redesign</li>", hasKitchen = locationObj.hasKitchen === "0" ? "" : "<li>Kitchen (" + parseSize(locationObj.hasKitchen) + ")</li>", hasBathroom = locationObj.hasBathroom === "0" ? "" : "<li>Bathroom (" + parseSize(locationObj.hasBathroom) + ")</li>", hasBalcony = locationObj.hasBalcony === "0" ? "" : "<li>Balcony</li>", hasAttic = locationObj.hasAttic === "0" ? "" : "<li>Attic</li>", hasPond = locationObj.hasPond === "0" ? "" : "<li>Pond</li>", hasCoop = locationObj.hasCoop === "0" ? "" : "<li>Small Animal Barn</li>", addRooms = locationObj.addRooms === "0" ? "" : "<li>Number of additional rooms: " + locationObj.addRooms + "</li>";
...
function submitGold() {
    ...
    var usernames = $("textarea#gold-users").val(), gold = parseInt($("input#gold-gold").val().replace(",", "")), link = $("input#gold-link").val();
    // handle error
    if (usernames === "") {
...
function submitMemory() {
    $("div#submit-memory-error").empty();
    $("div#submit-memory-result").empty();
    var user = $("input#memory-user").val(), key = $("select#memory-memory").val(), link = $("input#memory-link").val(), value;
    // handle error
    if (key === "") {
...
function toggleSeasonPrev() {
    var currSeason = parseInt($("td#day1").attr('data-season')), newSeason = currSeason === 0 ? 3 : currSeason - 1;
    constructBdayForSeason(newSeason);
}
function toggleSeasonNext() {
    var currSeason = parseInt($("td#day1").attr('data-season')), newSeason = currSeason === 3 ? 0 : currSeason + 1;
    constructBdayForSeason(newSeason);
}
function constructBdayForSeason(season) {
    var day = 1, seasonTitle = $("span#seasonTitle"), seasonColor, thead = $(".calendar > thead"), header = $(".calendar-header");
...
function populateHousing() {
    var keys = Object.keys(housingDB), startIndex = 0, houseKey = "", currKeyArr = [], housingSplit = {}, allHousing = [];
    ...
    for (var h = 0; h < allHousing.length; h++) {
        key = allHousing[h];
        if (key === "ai") {
            var innArray = housingSplit[key], leftLeft = 5, innId, innNum, innData, hasResidents, occupiedImgClass, emptyImgClass, rowCounter = 0, colCounter = 0;
            ...
            var ssArray = housingSplit[key], ssId, ssNum, ssData, hasResidents, occupiedImgClass, emptyImgClass, leftCounter = 0, rightCounter = 0;
            ...
            for (var i = 0; i < 6; i++) {
                ...
                top = (25 + 50 * i);
                for (var j = 0; j < 9; j++) {
                    var div = $('<div></div>'), houseId = housingSplit[key][i * 9 + j], houseData = housingDB[houseId], houseNum = houseData.address[1], hasResidents = houseData.residents !== "";
...
function createDialog(div, residentTitle, isItem) {
    if (residentTitle === void 0) { residentTitle = null; }
...
function getItemDialog(itemObj) {
    var itemName = itemObj.attr('data-id'), itemKey = itemObj.attr('data-key'), template = parseItemData(itemName, itemDB[itemKey][itemName]);
...
function getHouseDialog(houseObj, residentTitle) {
    var house = houseObj.attr('data-id'), houseObj = housingDB[house], template = parseLocation(houseObj, undefined, residentTitle);
    if (houseObj.img !== "") {
...
function calculateFP() {
    ...
    // collect int input
    var eventBonusInput = $("#fp-event").val().replace(",", ""), etcBonusInput = $("#fp-etc").val().replace(",", ""), eventBonus, etcBonus, hasArt = false, artBonus = 0, collabBonus, rpBonus, hqBonus, bdayBonus, itemBonus, total;
    if (eventBonusInput === "") {
        eventBonus = 0;
    }
    else {
...
function calculateRP() {
    ...
    var numRpersInput = $("#rp-num").val().replace(",", ""), wordCountInput = $("#rp-wc").val().replace(",", ""), numRpers, wordCount, total;
    if (numRpersInput === "") {
        numRpers = 0;
    }
    else {
...
function addUsername() {
    $("div#submit-gold-error").empty();
    $("div#submit-gold-result").empty();
    var user = $("select#submit-userList").val(), currentUsers = $("textarea#gold-users").val();
    if (user === "") {
